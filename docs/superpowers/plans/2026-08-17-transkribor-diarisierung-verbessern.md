...
```ts
  /** Laeuft die akustische Sprechertrennung auf diesem Server ueberhaupt? Bei
   *  `TRANSKRIBOR_DIARIZE=0` kehrt `cmd_diarize` sofort zurueck — das Feld „Anzahl Sprecher"
   *  waere dann ein Schalter, der gespeichert wird und nichts tut (#266). */
...
```tsx
  // `=== false` statt `!…`: ein aelterer Server ohne das Feld liefert `undefined`, und das
  // muss „laeuft" heissen — der Rueckfall geht zum bisherigen Verhalten, nicht in eine
...
CLUSTER_REGEL = (
    "ZWEI Cluster duerfen denselben Namen tragen — das ist eine ERLAUBTE Entscheidung, "
    "KEINE Fehlzuordnung. Bei Aufnahmen mit einem Kameramikrofon verteilt die Diarisierung "
    "denselben Menschen regelmaessig auf mehrere Cluster; sprechen zwei Cluster durchweg in "
    "Frageform, ist das derselbe Interviewer."
)
```

In `_correct_prompt` Regel 4 den Satz „Du DARFST zwei Cluster demselben Namen zuordnen, wenn klar dieselbe Person." ersetzen durch `{CLUSTER_REGEL}`.

In `_verify_prompt` die SPRECHER-Zeile (Z. 549) auf:

```
- SPRECHER: konsistent pro akustischem (Sprecher N)-Cluster und plausibel (Interviewer stellt Fragen; Antworten korrekt zugeordnet)? Fehlzuordnungen korrigieren. {CLUSTER_REGEL} Nicht auseinanderziehen — nur pruefen, ob es zutrifft.
```

In `_light_prompt` Schritt 3 und `_summary_prompt` Schritt 2 jeweils `{CLUSTER_REGEL}` an den Satz über die Namensvergabe anhängen.
...
`{CLUSTER_REGEL}` nacheinander aus je **einem** der vier Prompts entfernen ⇒ der erste Test muss jedes Mal rot werden und **den Namen des betroffenen Prompts nennen**. Danach „Kameramikrofon" aus der Konstante nehmen ⇒ zweiter Test rot. Alles zurückspielen.
...
`git stash` auf die Prompt-Änderung, dann:
...
`git stash pop`, abgeleitete Dateien im Wegwerf-Projekt löschen (`*.correction.json`, `*.edit.json`, `*.tagged.txt`, `*.md`), `run` erneut. **`--force` ist Pflicht**, sonst übernimmt der Block-Cache stillschweigend Teilblöcke nach der alten Regel.
...
In `fehlerquote` `dauer.get(i, 0.0)` durch `1.0` ersetzen ⇒ `test_fehlerquote_gewichtet_nach_ZEIT_nicht_nach_anzahl` rot. `linear_sum_assignment` durch eine feste Identitätszuordnung ersetzen ⇒ `test_fehlerquote_ist_unabhaengig_von_den_etiketten` rot. Beides zurückspielen, `__pycache__` leeren.

- [ ] **Step 7: Commit**

```bash
git add tools/diar_eval.py webtool/test_diar_eval.py .gitignore
...
- Produces: `eval/referenz.json` mit `{"dateien": {"<projekt>/<base>": {"projekt", "base", "sprecher_wahr": int, "segmente": [{"id", "start", "end", "sprecher"}]}}}` und `eval/laeufe/<name>.json` mit `{"name", "grenzen", "dateien": {"<schluessel>": {"sprecherzahl", "sprecher_wahr", "v", "homogenitaet", "vollstaendigkeit", "fehlerquote"}}}`

- [ ] **Step 1: `freeze` schreiben**

```python
def cmd_freeze(args) -> int:
    """Handkorrigierte edit.json -> eval/referenz.json.
    ...
    Genommen wird NUR, was `human_edited` traegt — eine LLM-vergebene Zuordnung ist keine
    Wahrheit (sie stammt teils aus denselben Clustern und kollabiert Gruppen unter ein
    Sammel-Etikett; gemessen an Rhyathlon/00111679, siehe Spec 1.3d).
    """
    dateien = {}
    for proj in sorted(os.listdir(args.projekte)):
        tdir = os.path.join(args.projekte, proj, "transkripte")
        if not os.path.isdir(tdir):
            continue
        for name in sorted(os.listdir(tdir)):
            if not name.endswith(".edit.json"):
                continue
            base = name[: -len(".edit.json")]
            with open(os.path.join(tdir, name), encoding="utf-8") as f:
                ed = json.load(f)
            if not ed.get("human_edited"):
                print(f"  ueberspringe {proj}/{base} (nicht handkorrigiert)")
                continue
            segmente = [{"id": s.get("id"), "start": s.get("start"), "end": s.get("end"),
                         "sprecher": s.get("speaker")}
                        for s in ed.get("segments", []) if s.get("speaker")]
            if not segmente:
                print(f"  ueberspringe {proj}/{base} (keine Sprecher)")
                continue
            dateien[f"{proj}/{base}"] = {
                "projekt": proj, "base": base,
                "sprecher_wahr": len({s["sprecher"] for s in segmente}),
                "segmente": segmente}
            print(f"  {proj}/{base}: {len(segmente)} Segmente, "
                  f"{dateien[f'{proj}/{base}']['sprecher_wahr']} Sprecher")
...
def cmd_run(args) -> int:
    ...
    for schluessel, eintrag in sorted(ref["dateien"].items()):
        ...
        ergebnis[schluessel] = {
            "sprecherzahl": sprecherzahl(vorhersage), "sprecher_wahr": eintrag["sprecher_wahr"],
            "homogenitaet": round(h, 4), "vollstaendigkeit": round(c, 4), "v": round(v, 4),
            "fehlerquote": round(fehlerquote(vorhersage, referenz, dauer), 4)}
        e = ergebnis[schluessel]
        print(f"  {schluessel:<44} {e['sprecherzahl']}/{e['sprecher_wahr']} Sprecher | "
              f"V {e['v']:.3f} | Fehler {e['fehlerquote']*100:5.1f}%", flush=True)
...
def _audio(wurzel: str, projekt: str, base: str):
    for ext in (".mp3", ".m4a", ".wav", ".mp4", ".aac", ".flac", ".ogg", ".opus", ".wma"):
...
def _summe(name: str, ergebnis: dict) -> dict:
    """Die drei Kennzahlen ueber den ganzen Satz. Die Fehlerquote wird NACH REDEZEIT gemittelt,
    nicht nach Dateien: sonst wiegt ein 40-Sekunden-Schnipsel so viel wie ein 5-Minuten-
    Interview, und der Satz waere durch seine kuerzesten Dateien bestimmt."""
    n = len(ergebnis) or 1
    ...
    summe = {"dateien": len(ergebnis), "zahl_getroffen": treffer,
             "v_mittel": round(sum(e["v"] for e in ergebnis.values()) / n, 4),
             "fehler_mittel": round(sum(e["fehlerquote"] for e in ergebnis.values()) / n, 4)}
    print(f"\n[{name}] {summe['dateien']} Dateien | Sprecherzahl getroffen: "
          f"{treffer}/{summe['dateien']} | V {summe['v_mittel']:.3f} | "
          f"Fehler {summe['fehler_mittel']*100:.1f}%")
    return summe
```
...
def main(argv=None) -> int:
    ...
    r.add_argument("name", help="Name des Laufs (-> eval/laeufe/<name>.json)")
...
Es gibt bereits vier (`Rhyathlon/00097495`, `US Car Treff/Fuhat Aras`, `Güldi Milo`, `Swiss Deuce Day…`). Damit lässt sich das Werkzeug prüfen, bevor Marcus' Arbeit vorliegt:

Run:
```bash
.venv/Scripts/python.exe tools/diar_eval.py freeze \
  --projekte "C:/Users/marcu/AppData/Roaming/Transkribor/projekte"
.venv/Scripts/python.exe tools/diar_eval.py run rauchprobe
```
Expected: vier Dateien in der Referenz, ein Lauf mit drei Zahlen je Datei, `eval/laeufe/rauchprobe.json` liegt da.
...
Expected: 13–14 Dateien, jede mit plausibler Sprecherzahl. Weicht eine Zahl von Marcus' Angabe ab, zuerst die Ursache klären — nicht die Zahl anpassen.
...
| C2 | Clustering `Fa`/`Fb`/`threshold` | Kopie der `config.yaml` nach `eval/configs/`, `run c2-fa07 --config …`. **`Fa` ist ungeprüft** — #264 testete nur `threshold` und `Fb`, und ohne Metrik |
| C3 | Einbettungs-Modell tauschen | `embedding:` in der config-Kopie auf das Alternativmodell, `run c3-<name> --config …` |
...
**C1–C4 sind reine Konfiguration** und in diesem Plan vollständig: eine Kopie der
`config.yaml` bzw. ein Kommandozeilen-Schalter, ein `run`, ein `vergleich`. Kein neuer Code,
...
1. Die **zeitgewichtete Fehlerquote** über den Satz sinkt.
...
def test_geaenderte_konfiguration_macht_ein_sidecar_ungueltig(project, monkeypatch):
    ...
    monkeypatch.setattr(diar, "konfig_fingerabdruck", lambda: "neu")
...
def test_gleiche_konfiguration_laesst_den_skip_greifen(project, monkeypatch):
    ...
    monkeypatch.setenv("TRANSKRIBOR_DIARIZE", "1")
    import webtool.diarize as diar
    monkeypatch.setattr(diar, "konfig_fingerabdruck", lambda: "gleich")
    (t / "S1.diar.json").write_text(json.dumps(
...
```python
def konfig_fingerabdruck() -> str:
    """Kurzer Hash ueber alles, was das Clustering-Ergebnis bestimmt (Modellpfad + Inhalt der
    config.yaml). Steht im Sidecar, damit ein Modell- oder Parameterwechsel bestehende
    Sidecars ungueltig macht — mtime und Sprecherzahl aendern sich dabei NICHT.

    `blake2b`, nicht `sha1`: letzteres geht durch OpenSSL und wirft unter einem FIPS-Provider
    (dieselbe Regel wie in ytdlp_update). Kein Wurf hier — die Funktion sitzt im Skip-Pfad
    eines Batch-Laufs.
    """
    import hashlib
    h = hashlib.blake2b(digest_size=8)
    h.update(os.path.abspath(DIAR_MODEL).encode("utf-8", "replace"))
    try:
        with open(DIAR_MODEL, "rb") as f:
            h.update(f.read())
    except OSError:
        pass          # fehlende config -> der Lauf scheitert gleich ohnehin, hier nicht werfen
    return h.hexdigest()
```

In `correct.cmd_diarize` die Skip-Bedingung um `and _sidecar_konfig(dpath) == fingerabdruck` erweitern und den Wert beim Schreiben mit ablegen. **`_sidecar_konfig` liefert bei fehlendem Schlüssel `None`** (gleiche Form wie `_sidecar_sprecher`, wirft nie) — ein Sidecar aus der Zeit vor diesem Feld gilt damit als „alte Konfiguration" und wird einmal neu gerechnet. Das ist die gewollte Folge, keine Regression.
...
```python
    ...
    fingerabdruck = diarize.konfig_fingerabdruck()   # EINMAL, nicht je Datei
```

Zwei Dinge dazu, die man nicht aus dem Diff liest: der bisherige Kommentar an dieser Stelle („lazy: zieht torch/pyannote erst hier") **stimmt nicht** — `webtool/diarize.py` importiert am Modulkopf nur `os`, die schweren Importe liegen in den Funktionen (steht so in seinem eigenen Docstring). Das Hochziehen kostet also nichts und ist gefahrlos; ein fehlendes pyannote schlägt weiterhin erst in `diarize_file` zu, innerhalb des bestehenden `try`. Und der Fingerabdruck liest die `config.yaml` — je Datei berechnet wäre das ein Dateizugriff pro Aufnahme für einen Wert, der sich im Lauf nicht ändert.
...
Die Fingerabdruck-Bedingung aus der Skip-Zeile entfernen ⇒ erste Zusicherung rot. `_sidecar_konfig` fest auf den eigenen Wert ⇒ ebenfalls rot. Zurückspielen, `__pycache__` leeren.

- [ ] **Step 6: Den Kandidaten selbst übernehmen**

Die gemessene Änderung einbauen (bei C1 eine Zeile in `correct.DIARIZE_MIN_SPEAKERS`, bei C2/C3/C4 die `models/…/config.yaml`, bei C3 zusätzlich das Modell in `models/` und die Zeile in `LICENSE-MODELLE.md`).
